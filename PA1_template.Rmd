...
```
The **mean** of the total number of steps taken per day is `r sprintf("%.2f", total_mean)`.   The **median** of the total number of steps taken per day is `r sprintf("%.2f", total_median)`.
...
```
The 5-minute interval with the maximum number of steps is interval `r max_interval[1]`, with `r sprintf("%.2f", max_interval[2])` steps.
...
```
We can see that the number of missing values is `r missing_values`. 
...
```
The **mean** of the total number of steps taken per day is `r sprintf("%.2f", total_mean_imputed)`. The **median** of the total number of steps taken per day is `r sprintf("%.2f", total_median_imputed)`. We can see that the **mean** and **median** values of the imputed data set are equal and they are different for raw data set.   
